...
| Cloudflare (Turnstile) | Cloudflare, Inc. | Bot / spam / abuse protection (CAPTCHA challenge) on public forms such as signup | Provider-managed global edge network | No — only on abuse-protected public forms when Turnstile is enabled | Legitimate interest (Art. 6(1)(f) GDPR) in security/abuse prevention. Cloudflare DPA + SCCs cover third-country processing. Processes IP + coarse browser signals; Cloudflare states Turnstile is not used for cross-site tracking or advertising. |
...
| OpenAI | OpenAI OpCo LLC | Optional LLM inference for agent nodes / model ops; **text embeddings for agent knowledge retrieval (platform key)** | US by default; EU residency only for eligible configured API projects | Embeddings: yes when the platform key is configured, **except EU-only workspaces** (keyword retrieval instead). Inference: only when selected or a customer key is configured | DPA + SCCs unless an eligible EU-resident project is verified. Knowledge text and search queries receive structured-identifier redaction before embedding; EU-only workspaces send nothing to OpenAI unless the platform project is verified EU-resident. |
...
Structured identifiers (email addresses, phone numbers, IBANs, IP addresses, national IDs, payment card numbers) are **pseudonymized** before prompts leave Corelyx infrastructure: the model only sees stable numbered placeholders (e.g. `[EMAIL_1]`), and the re-identification mapping never leaves the EU runtime — real values are substituted back into outputs and connector actions on Corelyx's side. Secrets and credentials are destructively redacted and never restored. Workspaces on the **strict privacy tier** (the default for EU-only workspaces where the runtime has a local NER backend installed) additionally pseudonymize **person names** detected entirely on Corelyx servers. Other free-text prose is **not** removed.
...
| Date | Change |
|---|---|
| 2026-07-24 | Added Cloudflare (Turnstile) as a bot/abuse-protection subprocessor (Art. 6(1)(f)); previously undisclosed despite being loaded on signup and allowed in the CSP. |
