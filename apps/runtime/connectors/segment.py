...
_BASE = "https://api.segment.com/v1"


class SegmentConnector(IConnector):
    """
    Segment connector for: track_event, identify_user.
            ...
            match operation:
                ...
                case _:
                    raise ConnectorError(
                        ...
                        f"Segment does not support '{operation}'",
                    )
    ...
    async def _identify_user(
        ...
        r = await request_with_rate_limit(
            client, "POST", f"{_BASE}/identify_user",
            headers=headers, json=params or {},
        )
        if r.status_code >= 400:
            raise ConnectorError("API_ERROR", r.text)
