...
class DeelConnector(IConnector):
    ...
    provider = "deel"
    supported_operations = ["list_contractors", "get_contractor"]
    ...
    async def _list_contractors(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
    ...
    async def _get_contractor(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
        """Execute get_contractor operation."""
        r = await request_with_rate_limit(
            client,
            "POST",
            f"{_BASE}/get_contractor",
            headers=headers,
            json=params or {},
        )
        if r.status_code >= 400:
