...
class GrafanaConnector(IConnector):
    ...
    provider = "grafana"
    supported_operations = ["list_dashboards", "query_metrics"]
    ...
    async def _list_dashboards(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
    ...
    async def _query_metrics(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
        """Execute query_metrics operation."""
        r = await request_with_rate_limit(
            client,
            "POST",
            f"{_BASE}/query_metrics",
            headers=headers,
            json=params or {},
        )
        if r.status_code >= 400:
