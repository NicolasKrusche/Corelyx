...
class CloudinaryConnector(IConnector):
    ...
    provider = "cloudinary"
    supported_operations = ["list_resources", "upload_resource"]
    ...
    async def _list_resources(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
    ...
    async def _upload_resource(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
        """Execute upload_resource operation."""
        r = await request_with_rate_limit(
            client,
            "POST",
            f"{_BASE}/upload_resource",
            headers=headers,
            json=params or {},
        )
        if r.status_code >= 400:
