...
_BASE = "https://api.kustomer.com/v1"


class KustomerConnector(IConnector):
    """
    Kustomer connector for: list_customers, create_customer.

    API Base: kustomer
    """

    provider = "kustomer"
    supported_operations = ["list_customers", "create_customer"]
    ...
    async def _create_customer(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
        """Execute create_customer operation."""
        if not params:
