...
class NetlifyConnector(IConnector):
    ...
    provider = "netlify"
    supported_operations = ["list_sites", "trigger_build"]
    ...
    async def _trigger_build(self, client: httpx.AsyncClient, headers: dict, params: dict) -> dict:
        """Execute trigger_build operation."""
        r = await request_with_rate_limit(
            client,
            "POST",
            f"{_BASE}/trigger_build",
            headers=headers,
            json=params or {},
        )
        if r.status_code >= 400:
