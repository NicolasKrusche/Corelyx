...
class FreeModelSuiteTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_trigger_cron_schema_and_execution(self) -> None:
        trigger = _node("t1", "trigger", TriggerConfig(trigger_type="cron", extra={"expression": "0 8 * * *", "timezone": "UTC"}))
        step = _node("s1", "step", StepConfig(logic_type="transform", extra={"transformation": "input['value'] + 1"}))
        program = _program([trigger, step], [_edge("e1", "t1", "s1", "data_flow")])
        result = await self._run_executor(_executor(program), {"value": 10})
        self.assertEqual(result["s1"]["result"], 11)
    ...
    async def test_trigger_webhook_schema_and_execution(self) -> None:
        trigger = _node("t1", "trigger", TriggerConfig(trigger_type="webhook", extra={"endpoint_id": "ep-test", "method": "POST"}))
        step = _node("s1", "step", StepConfig(logic_type="transform", extra={"transformation": "input['value'] + 1"}))
        program = _program([trigger, step], [_edge("e1", "t1", "s1", "data_flow")])
        result = await self._run_executor(_executor(program), {"value": 10})
        self.assertEqual(result["s1"]["result"], 11)
    ...
    async def test_trigger_event_schema_and_execution(self) -> None:
        trigger = _node("t1", "trigger", TriggerConfig(trigger_type="event", extra={"source": "gmail", "event": "new_email", "filter": {}}))
        step = _node("s1", "step", StepConfig(logic_type="transform", extra={"transformation": "input['value'] + 1"}))
        program = _program([trigger, step], [_edge("e1", "t1", "s1", "data_flow")])
        result = await self._run_executor(_executor(program), {"value": 10})
        self.assertEqual(result["s1"]["result"], 11)
    ...
    async def test_trigger_program_output_schema_and_execution(self) -> None:
        trigger = _node("t1", "trigger", TriggerConfig(trigger_type="program_output", extra={"source_program_id": "p2", "on_status": ["success"]}))
        step = _node("s1", "step", StepConfig(logic_type="transform", extra={"transformation": "input['value'] + 1"}))
        program = _program([trigger, step], [_edge("e1", "t1", "s1", "data_flow")])
        result = await self._run_executor(_executor(program), {"value": 10})
        self.assertEqual(result["s1"]["result"], 11)
    ...
    async def test_step_sort_asc(self) -> None:
        ...
        result = await _executor(_program([node], []))._execute_node(node, {"items": [{"name": "Z"}, {"name": "A"}, {"name": "M"}]})
    ...
    async def test_oauth_connection_schema_for_every_provider(self) -> None:
        ...
        for name, cls in sorted(REGISTRY.items()):
            with self.subTest(provider=name):
                ...
                safe_ops = [op for op in inst.supported_operations if any(op.startswith(p) for p in ("list_", "get_", "read_", "query_", "search_"))]
    ...
    async def test_advanced_full_program_with_all_step_types(self) -> None:
        ...
        result = await self._run_executor(_executor(program), {"value": 10, "raw_json": '[{\"id\":1,\"name\":\"Z\"},{\"id\":2,\"name\":\"A\"},{\"id\":1,\"name\":\"Z\"}]'})
    ...
    async def test_advanced_branching_with_retries(self) -> None:
        ...
        high = _node("high", "step", StepConfig(logic_type="format", extra={"template": "High: {score}", "output_key": "label"}))
        medium = _node("medium", "step", StepConfig(logic_type="format", extra={"template": "Medium: {score}", "output_key": "label"}))
        low = _node("low", "step", StepConfig(logic_type="format", extra={"template": "Low: {score}", "output_key": "label"}))
    ...
    async def test_advanced_error_handling_filter_blocks(self) -> None:
        trigger = _node("t1", "trigger", TriggerConfig(trigger_type="manual"))
        filter_step = _node("filter", "step", StepConfig(logic_type="filter", extra={"condition": "input['value'] > 0"}))
        downstream = _node("down", "step", StepConfig(logic_type="format", extra={"template": "Val: {value}", "output_key": "msg"}))
        edges = [
    ...
    async def test_advanced_agent_retry_config(self) -> None:
        node = _node("a1", "agent", AgentConfig(
            ...
            scope_required=None,
            scope_access="read",
            retry=RetryConfig(max_attempts=3, backoff="exponential", backoff_base_seconds=2, fail_program_on_exhaust=True),
            tools=[],
        ))
        executor = _executor(_program([node], []))
        executor._fetch_api_key = AsyncMock(return_value=("fake-key", "openrouter"))
    ...
    async def test_placeholder_skipped_destructive_ops(self) -> None:
        ...
        for name, cls in sorted(REGISTRY.items()):
            inst = cls()
            destructive = [op for op in inst.supported_operations if any(op.startswith(p) for p in (
                "create_", "send_", "update_", "delete_", "post_", "put_", "patch_", "upload_", "append_", "move_",
                "write_", "publish_", "submit_", "trigger_", "archive_", "add_", "insert_", "replace_",
                "schedule_", "cancel_", "clear_", "run_", "execute_", "sync_", "enqueue_", "launch_",
                "set_", "edit_", "modify_", "change_", "complete_", "track_", "identify_", "label_", "reply_",
                "invite_", "start_", "stop_", "clone_", "copy_", "upsert_", "reset_", "revoke_", "install_",
                "uninstall_", "unsubscribe_", "kick_",
            ))]
