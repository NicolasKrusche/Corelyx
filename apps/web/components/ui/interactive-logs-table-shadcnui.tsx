...
function LogRow({
  ...
  expanded: boolean;
  onToggle: () => void;
  renderDetail?: (log: Log) => React.ReactNode;
}) {
  ...
  return (
    <>
      <div className="w-full p-4 transition-colors hover:bg-muted/50">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex-shrink-0 inline-flex items-center gap-1.5 rounded-md border border-border bg-background px-2.5 py-1 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
          >
            ...
            {renderDetail ? (
              ...
              <div className="space-y-4 p-4">
                <div>
                  ...
                  <p className="rounded bg-background p-3 font-mono text-sm text-foreground whitespace-pre-wrap break-words">
  ...
  return (
    ...
    >
      ...
      <FilterGroup
        ...
        onToggle={(v) => toggleFilter("service", v)}
      />
      <FilterGroup
        title="Status"
        values={statuses}
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      ...
      const matchSearch =
        ...
        (log.detail ?? "").toLowerCase().includes(lowerQuery);
