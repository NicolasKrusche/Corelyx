...
const CONNECTOR_DEFINITIONS: Record<string, ConnectorDef> = {
  ...
  list_emails / search: params={query:string(REQUIRED),max_results:number} → output:{emails:[{id,threadId}]}
    ...
    ⚠ REQUIRED PATTERN when processing individual emails: list_emails(nX) → filter(data['nX'].get('emails',[])>0) → loop(over:"data['nX']['emails']",item_var:"email") → read_email(message_id:"{{nX.email.id}}") → [branch/filter/agent using data['read_node']['subject'] etc.]
