...
export const providerRegistry: Provider[] = [
  ...
  },
  {
    ...
    category: "security",
    purpose: "Bot, spam, and abuse protection (CAPTCHA challenge) on public forms such as signup. Only active when Turnstile is enabled via the NEXT_PUBLIC_TURNSTILE_SITE_KEY / TURNSTILE_SECRET_KEY configuration.",
    default_region: "Provider-managed global edge network.",
    available_regions: ["Provider-managed global edge network"],
    eu_only_supported: false,
    data_categories_processed: ["IP address", "Browser and device signals", "Challenge interaction data", "Turnstile token"],
    personal_data_allowed: true,
    stores_prompts: false,
    stores_outputs: false,
    trains_on_customer_data: false,
    dpa_available: true,
    scc_available: true,
    transfer_basis: "Processed on Cloudflare's global edge as a bot/abuse safeguard under Art. 6(1)(f) GDPR (legitimate interest in security). Cloudflare DPA and SCCs cover any third-country processing.",
    subprocessor_url: "https://www.cloudflare.com/gdpr/subprocessors/",
    privacy_url: "https://www.cloudflare.com/privacypolicy/",
    retention_notes: "Cloudflare states Turnstile does not use the data for cross-site tracking or advertising; challenge data is retained only briefly for abuse prevention per Cloudflare's retention schedule.",
    last_reviewed_at: "2026-07-24",
    status: "warning",
    optional: false,
    used_by_default: false,
    activation: "Only on abuse-protected public forms (e.g. signup) when Turnstile is enabled",
  },
  {
    id: "resend",
    name: "Resend",
