...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "drip:primary";

  const tokenRes = await fetch("https://www.getdrip.com/oauth/token", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({
      code,
      client_id: process.env.DRIP_CLIENT_ID!,
      client_secret: process.env.DRIP_CLIENT_SECRET!,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=drip`, flowId);
}
