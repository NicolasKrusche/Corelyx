...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "calendly:primary";
  ...
  const tokenRes = await fetch("https://auth.calendly.com/oauth/token", {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/calendly/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=calendly`, flowId);
}
