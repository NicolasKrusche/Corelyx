...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "loom:primary";

  const tokenRes = await fetch("https://www.loom.com/v2/oauth2/token", {
    method: "POST",
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=loom`, flowId);
}
