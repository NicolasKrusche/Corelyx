...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "mailchimp:primary";
  ...
  const tokenRes = await fetch("https://login.mailchimp.com/oauth2/token", {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/mailchimp/callback`,
  ...
  try {
    await upsertOAuthConnection(serviceClient, {
      ...
      metadata: { dc: tokens.dc },
    });
  } catch {
    return redirectWithClearedOAuthState(`${origin}/connections?error=vault_failed`, flowId);
  }

  return redirectWithClearedOAuthState(`${origin}/connections?connected=mailchimp`, flowId);
}
