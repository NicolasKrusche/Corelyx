...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "constantcontact:primary";

  const credentials = Buffer.from(`${process.env.CONSTANTCONTACT_CLIENT_ID!}:${process.env.CONSTANTCONTACT_CLIENT_SECRET!}`).toString("base64");
  const tokenRes = await fetch("https://authz.constantcontact.com/oauth2/default/v1/token", {
    ...
    body: new URLSearchParams({
      code,
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/constantcontact/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=constantcontact`, flowId);
}
