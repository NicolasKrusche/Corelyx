...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "instagram:primary";
  ...
  const tokenRes = await fetch("https://api.instagram.com/oauth/access_token", {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/instagram/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=instagram`, flowId);
}
