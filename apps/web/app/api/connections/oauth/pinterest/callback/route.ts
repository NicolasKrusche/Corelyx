...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "pinterest:primary";
  ...
  const tokenRes = await fetch("https://api.pinterest.com/v5/oauth/token", {
    ...
    body: new URLSearchParams({
      code,
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/pinterest/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=pinterest`, flowId);
}
