...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "salesforce:primary";
  ...
  const tokenRes = await fetch("https://login.salesforce.com/services/oauth2/token", {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/salesforce/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=salesforce`, flowId);
}
