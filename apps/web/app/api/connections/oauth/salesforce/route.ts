...
export async function GET(request: Request) {
  ...
  const label = searchParams.get("label") ?? "salesforce:primary";
  ...
  const response = NextResponse.redirect(`https://login.salesforce.com/services/oauth2/authorize?${params.toString()}`);
