...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "front:primary";

  const tokenRes = await fetch("https://app.frontapp.com/oauth/token", {
    method: "POST",
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=front`, flowId);
}
