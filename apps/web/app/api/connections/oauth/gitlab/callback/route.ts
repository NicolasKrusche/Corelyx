...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "gitlab:primary";
  ...
  const tokenRes = await fetch("https://gitlab.com/oauth/token", {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/gitlab/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=gitlab`, flowId);
}
