...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "height:primary";

  const tokenRes = await fetch("https://api.height.app/oauth/tokens", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({
      code,
      client_id: process.env.HEIGHT_CLIENT_ID!,
      client_secret: process.env.HEIGHT_CLIENT_SECRET!,
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/height/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=height`, flowId);
}
