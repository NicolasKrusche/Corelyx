...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "nifty:primary";
  const tokenRes = await fetch("https://niftypm.com/oauth/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ code, client_id: process.env.NIFTY_CLIENT_ID!, client_secret: process.env.NIFTY_CLIENT_SECRET!, redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/nifty/callback`, grant_type: "authorization_code" }), cache: "no-store" });
  ...
  try { await upsertOAuthConnection(serviceClient, { userId, provider: "nifty", label, tokens: { access_token: accessToken, refresh_token: tokens.refresh_token as string | undefined }, scopes: [], metadata: {} }); }
  catch { return redirectWithClearedOAuthState(`${origin}/connections?error=vault_failed`, flowId); }
  return redirectWithClearedOAuthState(`${origin}/connections?connected=nifty`, flowId);
}
