...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "freshdesk:primary";

  const tokenRes = await fetch("https://accounts.freshdesk.com/oauth/token", {
    method: "POST",
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=freshdesk`, flowId);
}
