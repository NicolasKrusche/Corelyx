...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "teamwork:primary";

  const tokenRes = await fetch("https://www.teamwork.com/launchpad/v1/token.json", {
    method: "POST",
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/teamwork/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=teamwork`, flowId);
}
