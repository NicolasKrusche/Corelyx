...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "zohoprojects:primary";
  const tokenRes = await fetch("https://accounts.zoho.com/oauth/v2/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ code, client_id: process.env.ZOHO_CLIENT_ID!, client_secret: process.env.ZOHO_CLIENT_SECRET!, redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/zohoprojects/callback`, grant_type: "authorization_code" }), cache: "no-store" });
  ...
  try { await upsertOAuthConnection(serviceClient, { userId, provider: "zohoprojects", label, tokens: { access_token: accessToken, refresh_token: tokens.refresh_token as string | undefined }, scopes: [], metadata: {} }); }
  catch { return redirectWithClearedOAuthState(`${origin}/connections?error=vault_failed`, flowId); }
  return redirectWithClearedOAuthState(`${origin}/connections?connected=zohoprojects`, flowId);
}
