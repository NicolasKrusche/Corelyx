...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "buffer:primary";

  const tokenRes = await fetch("https://api.bufferapp.com/1/oauth2/token", {
    method: "POST",
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=buffer`, flowId);
}
