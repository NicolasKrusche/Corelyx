...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "linear:primary";
  ...
  const tokenRes = await fetch("https://api.linear.app/oauth/token", {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${process.env.NEXT_PUBLIC_APP_URL}/api/connections/oauth/linear/callback`,
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=linear`, flowId);
}
