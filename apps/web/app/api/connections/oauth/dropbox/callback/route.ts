...
export async function GET(request: Request) {
  ...
  const label = typeof payload.label === "string" && payload.label.trim() ? payload.label : "dropbox:primary";

  const credentials = Buffer.from(`${process.env.DROPBOX_CLIENT_ID!}:${process.env.DROPBOX_CLIENT_SECRET!}`).toString("base64");
  ...
  return redirectWithClearedOAuthState(`${origin}/connections?connected=dropbox`, flowId);
}
