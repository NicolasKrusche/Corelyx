...
export function generateStaticParams() {
  return getSeoPagesBySection("integrations").map((page) => ({
    slug: page.path === "/integrations" ? [] : page.path.replace("/integrations/", "").split("/"),
  }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug = [] } = await params;
  const page = getSeoPage(pathFromParts("integrations", slug));
  if (!page) notFound();
